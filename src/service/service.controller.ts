import { Controller, Post, Body, Param, Delete, Get, Patch, Query, Req, ParseIntPipe} from '@nestjs/common';
...
export class ServiceController {
  ...
  @Get('all_services')
  @UseGuards(JwtAuthGuard, RolesGuard)
    async get_allServices(@Query() dto: FilterServiceDTO) {
      return await this.serviceService.get_all_services(dto);
    }

  @Patch('update_service/:id')
  @UseGuards(JwtAuthGuard, RolesGuard)
    async update_service(@Param('id',ParseIntPipe) id: string, @Body() dto: UpdateServiceDto, @Req() req: Request){
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    async delete_service(@Param('id', ParseIntPipe) id: string) {
      //console.log(id);
